...
class TestSignerFactory(unittest.TestCase):
    ...
    def test_credential(self):
      ...
      # sts token signer
      cred = {'credential': StsTokenCredential('sts_access_key_id', 'sts_access_key_secret', 'sts_token')}
      signer = SignerFactory.get_signer(cred, 'cn-hangzhou', 'do-action-api')
      self.assertIsInstance(signer, StsTokenSigner)

      # Ram Role Arn Credential
      cred = {'credential': RamRoleArnCredential('sts_access_key_id', 'sts_access_key_secret', 'role_arn', 'session_role_name')}
      signer = SignerFactory.get_signer(cred, 'cn-hangzhou', 'do-action-api')
      ...
      # public_key_path = os.path.join(os.path.dirname(__file__), "..", "..", "fixtures", "id_rsa.pub")
      # public_key_id = open(public_key_path).read()
      # private_key_path = os.path.join(os.path.dirname(__file__), "..", "..", "fixtures", "id_rsa")
      # private_key_id = open(private_key_path).read()

      # cred = {'credential': RsaKeyPairCredential(public_key_id, private_key_id, session_period=3600)}
      # signer = SignerFactory.get_signer(cred, 'cn-hangzhou', 'do-action-api')
