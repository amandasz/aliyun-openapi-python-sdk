...
class TestShaHmac1(unittest.TestCase):
    ...
    def test_md5_sum(self):
      self.assertEqual("ERIHLmRX2uZmssDdxQnnxQ==", helper.md5_sum("That's all folks!!"))
    ...
    def test_get_iso_8061_date(self):
      ...
      self.assertRegexpMatches(s, '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z$')

    def test_get_rfc_2616_date(self):
      s = helper.get_rfc_2616_date()
      self.assertEqual(29, len(s))
      self.assertRegexpMatches(s, '^[A-Z][a-z]{2}, [0-9]{2} [A-Z][a-z]{2} [0-9]{4} [0-9]{2}:[0-9]{2}:[0-9]{2} GMT$')
